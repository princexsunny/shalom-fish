...
**Live Media:** an **autoplaying reel inside the tile** — videos and photos, no
play button, no pause control, no tap-to-open and no fullscreen viewer. Videos
play muted to their natural end (`onEnded`), photos hold **4.5s**, then it
advances and cross-fades (`.media-fade`, 350ms). A long or broken clip is capped
at **15s** so the reel can't stall; a single item just loops. `playsInline`,
`object-cover`; pauses off-screen (`IntersectionObserver`). A small chip
top-left shows `12s` / `Photo`. Segment bars bottom-right show position
(max 6 + "+N"). Tapping is only active when the item is **linked to a product** —
then it jumps the carousel to that fish. Unlinked items are inert.
